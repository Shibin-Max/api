...
@DirtiesContext
class LNWChannelStrategyTest {
    private Logger logger = LoggerFactory.getLogger("log");

    @Resource
    private LNWChannelStrategy lnwChannelStrategy;

    private final PlatformEnum platform = PlatformEnum.LNW;
...
//        TInBetSummaryRecord inOrdersSummary = ezeeChannelStrategy.getInOrdersSummary(param);
        TInBetSummaryRecord sss = lnwChannelStrategy.getInOrdersSummary(param);
    }
    ...
    void getInOrders() {
        ...
        TimeRangeParam param = TimeRangeParam.from(start, end);
        InOrdersResult inOrders = lnwChannelStrategy.getInOrders(param);
    ...
    void getOutOrders() {
        ...
        // 调用渠道策略方法
        LobbyOrderResult outOrders = lnwChannelStrategy.getOutOrders(param);
    ...
    void getOutOrderSummary() {
        ...
        // 调用汇总方法
        TOutBetSummaryRecord outOrders = lnwChannelStrategy.getOutOrdersSummary(param);
