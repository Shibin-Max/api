package net.tbu.spi.strategy.channel.impl.gemini.base;
...
/**
 * 厅方汇总接口和明细接口时间区间是
 * DC 汇总统计还是明细统计 都是<=  <  [x,y)  2025-05-19 10:00:00  2025-05-19 11:00:00
 * 每天的数据量 <10000
 * Gemini tom
 */
@Slf4j
@Service
public abstract class GeminiBaseChannelStrategy extends BaseChannelStrategy {

    @Resource
    protected ThirdPartyGatewayFeignService gatewayFeignService;

    // GEMM厅方接口url
    protected static final String GEMM_LIST = "/result/query/";
    protected static final String GEMM_SUMMARY = "/summary/hourly/";

    protected static final int GEMM_PAGE_LIMIT = 10000;
    ...
    public TOutBetSummaryRecord getOutOrdersSummary(TimeRangeParam param) {
        ...
        log.info("[GEMINI.getOutOrdersSummary][step1] [channelName({})] [time({})]", channelName, param);
        ...
        if (result.getCode() != 0 && result.getCode() != 931) {
            throw new CustomizeRuntimeException(String.format("%s GeminiBaseChannelStrategy getOutOrdersSummary(汇总) method two response error %s", channelName, result.getMessage()));
        } else {
            list = ObjectUtil.defaultIfNull(result.getData(), Collections.emptyList());
        }

        log.info("[GEMINI.getOutOrdersSummary][step2] [channelName({})] [size({})] [lobbyReq({})] getOutOrdersSummary(汇总)", channelName, list.size(), lobbyReq);
        ...
        log.info("[GEMINI.getOutOrdersSummary][step3] [channelName({})] [start({})] [end({})] [outBetAmountSum({})] [outEffBetAmountSum({})] [outSumWlValue({})] [outSumUnitQuantity({})]",
    ...
    public LobbyOrderResult getOutOrders(TimeRangeParam param) {
        log.info("[GEMINI][START]  [channelName({})] [param({})] ", channelName, param);
        ...
        log.info("[GEMINI][END] [channelName({})] [param({})] [resultSize({})] 获取明细总数", channelName, param, result.size());
        return result;
    }
    ...
    private  MutableList<GEMINILobbyResp.GEMMRecord> getLobbyOrders(TimeRangeParam param, GEMINILobbyReq req) {
        req.setHttpMethod(HttpMethod.GET.name());
        req.setPlatformId(getChannelType().getPlatformId());
        req.setUri(GEMM_LIST + GEMM_REQ_DT_FMT.format(param.start().withZoneSameInstant(GEMM_ZONE_OFFSET)));
        req.setTimeSlug(GEMM_REQ_DT_FMT.format(param.start().withZoneSameInstant(GEMM_ZONE_OFFSET)));
        log.info("[GEMINI.getLobbyOrders][req] [channelName({})] [req({})] ", channelName, req);
        int page = 0;
        ...
        do {
            ...
            orders = Optional.of(req)
                    ...
                    .map(json -> {
                        ...
                        log.info("[GEMINI.getLobbyOrders][step1] [channelName({})] [param({})] [valPage({})] [httpMethod({})], [platformId({})], [uri({})], [timeSlug({})], [返回JSON({})]",
                                channelName, param, valPage, req.getHttpMethod(), req.getPlatformId(), req.getUri(), req.getTimeSlug(), truncatedJson);
                        return parseObject(json, GEMINILobbyResp.class);
                    })
                    .map(s -> {
                        if (s.getCode() != 0) {
                            throw new CustomizeRuntimeException(String.format("%s GeminiBaseChannelStrategy getOutOrders method two response error %s", channelName, s.getMessage()));
                        }
                        return s;
                    })
            ...
            for (GEMINILobbyResp.GEMMRecord record : orders) {
                log.info("[GEMINI.getLobbyOrders][record] status({}), bet_id({}), bet_num({}), confirmed_at({}), settled_at({}), bet_at({}), player({}), provider({}), category({}), channel({}), match_id({}), bet_amount({}), valid_amount({}), net_income({}), bet_return({}), locale_bet_type({}), locale_channel({}), locale_game_result({})",
                        record.getStatus(),
                        record.getBet_id(),
                        record.getBet_num(),
            ...
            log.info("[GEMINI.getLobbyOrders][step3] [channelName({})] [param({})] [page({})] [orderSize({})]", channelName, param, page, orders.size());
            ThreadUtil.sleep(GEMM_PAGE_LIMIT);
