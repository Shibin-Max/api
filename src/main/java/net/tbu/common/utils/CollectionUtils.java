...
public class CollectionUtils {
    ...
    /**
     * 对集合中某个数值字段求和，支持 Integer 和 Long 类型，null 值视为 0。
     *
     * @param from      原始集合
     * @param valueFunc 提取数值字段的函数（返回 Number，可为 Integer 或 Long）
     * @return 所有元素对应值的总和
     */
    public static <T> Long getSumValueAsLong(Collection<T> from, Function<T, ? extends Number> valueFunc) {
        if (CollUtil.isEmpty(from)) {
            return 0L;
        }
        return from.stream()
